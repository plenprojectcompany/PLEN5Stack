...
PLEN5Stack::PLEN5Stack()***REMOVED******REMOVED***
...
***REMOVED***
    ...
    ***REMOVED***
        ...
        if(! str.equals(">MF")) ***REMOVED*** break; ***REMOVED***
        listNum += 3; // >MF
        str = mFile.substring(listNum, listNum+2);
        if (fileName != strtol(str.c_str(), NULL, 16))***REMOVED*** break; ***REMOVED***
        ...
        for (uint8_t val = 0; val < SERVO_NUM_USED; val++)
        ***REMOVED***//plen2 max servo = 24, plenbit = 7
            str = mFile.substring(listNum, listNum+4);
